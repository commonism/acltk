...
from acltk.aclObjects import (
    TimeRangeObjectAbsolute,
    TimeRangeObjectPeriodic,
    TimeRange,
    NetworkObject,
    ServiceObject,
    NetworkGroup,
    ...
    NATMapped,
    NATMappedSource,
    NATMappedSourceFallback,
    NATMappedDestination,
    NATReal,
    NATRealNode,
    Name,
)
...
class aclSemantics:
    ...
    def time_range_object(self, ast):
        assert ast.type in frozenset(["no", "absolute", "periodic"]), f"time-range type {ast.type} is not known"
        if ast.type == "no":
            return None
    ...
    def object_group_type(self, ast):
        action = {
            ...
            "tcp-udp": (
                lambda name, d: PortGroup(name, Protocol("tcp-udp"), description=d),
                self.parser.service_groups,
            ),
        ...
        assert isinstance(ast.type, (str, Protocol)), f"unknown type {type(ast.type)}"
    ...
    def protocol_int(self, ast):
        assert 0 <= int(ast) <= 255, f"invalid protocol {ast}"
        return Protocol(ast)

    def port_int(self, ast):
        assert 0 <= int(ast) <= 2**16 - 1, f"invalid port {ast}"
        return ast

    def icmp_type_int(self, ast):
        assert 0 <= int(ast) <= 2**8 - 1, f"invalid icmp type {ast}"
        return ast

    def icmp_code_int(self, ast):
        assert 0 <= int(ast) <= 2**8 - 1, f"invalid icmp code {ast}"
        return ast
    ...
    def service_group_object(self, ast):
        ...
        if ast.type == "service-object":
            if ast.protocol == "object":
                ...
                assert isinstance(ast.protocol, Protocol), f"protocol {ast.protocol} is unknown"
    ...
    def service_object(self, ast):
        assert isinstance(ast["protocol"], Protocol), f"invalid protocol {ast.protocol}"
        return ast
    ...
    def acl_protocol(self, ast):
        if ast.type == "object":
            ...
            assert isinstance(ast.name, Protocol), f"invalid type {type(ast.name)}"
            return ast.name
    ...
    def acl_options(self, ast):
        r = {}
        for i in ast:
            assert i.type is None or i.type in frozenset(
                ["log", "time-range", "inactive"]
            ), f"option {i.type} is unknown"
            if i.type is None:
                continue
            elif i.type == "log":
    ...
    def network_object_nat(self, ast):
        ...
        if fallback and fallback.interface == "interface":
            fallback.interface = (
                mapped["src"]
                if isinstance(mapped["src"], NetworkInterface)
                else NetworkInterface(mapped["iface"].nameif or mapped["iface"].alias)
            )
        ...
        r = NATObject(
            NATReal(real["iface"], NATRealNode(real["src"]), NATRealNode(real["dst"]), mapped.get("service", None)),
            NATMapped(
                mapped["iface"],
