...
class pfsenseParser(aclParser):
    ...
    def _parse_system(self, root):
        ...
        for i in list(root):
            if i.tag == "domain":
                ...
                hostname = i.text
        ast.append(tatsu.ast.AST(**{"hostname": f"{hostname}.{domain}"}))
        return ast
        ...
        for i in list(root):
            ...
            if "ipaddr" in values:
                if values["ipaddr"] != "dhcp":
                    details = [
                        tatsu.ast.AST(**{"type": ["ip", "address"], "value": [values["ipaddr"], values["subnet"]]})
                    ]
                else:
            ...
            if alias.startswith("opt"):
                ...
                details.append(
                    tatsu.ast.AST(**{"type": "description", "value": html.parser.unescape(values.get("descr", ""))})
                )
    ...
    def _parse_filter(self, root):
        ...
        for i in list(root):
            if i.tag == "rule":
                ...
                separators = collections.defaultdict(lambda: collections.defaultdict(list))
                for ifname in list(i):
                    for sep in list(ifname):
                        ...
                        separators[iface][row].append(
                            ACLCaption(bg=values["color"], text=html.parser.unescape(values["text"]))
                        )
    ...
    def _parse_filter__rule(self, i, now):
        ...
        values = {
            ...
            "destination": ACLNode(NetworkAny()),
            "created": now,
            "updated": now,
            "log": False,
        ...
        for v in list(i):
            if v.tag in valueable:
                ...
                raise ValueError(f"unknown data {v.tag}")
        ...
        dates = "c: {} ({} days) / m: {} ({} days)".format(
            values["created"].strftime("%Y-%m-%d"),
