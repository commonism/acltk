...
class fwsmParser(Parser):
    ...
    def _command_(self):
        with self._choice():
            ...
            self._error(
                'expecting one of: '
                "'!' ':' 'Cryptochecksum:' 'aaa' 'aaa-"
                "server' 'access-group' 'access-list'"
                "'app-agent' 'arp' 'arp-inspection'"
                "'asdm' 'auth-prompt' 'boot' 'call-home'"
                "'class-map' 'clock' 'command-alias'"
                "'console' 'crypto' 'dhcpd' 'dhcprelay'"
                "'dns' 'dns-guard' 'domain-name'"
                "'dynamic-access-policy-record' 'enable'"
                "'eou' 'established' 'failover'"
                "'firewall' 'flow-export' 'fragment'"
                "'ftp' 'global' 'group-policy' 'hostname'"
                "'hpm' 'http' 'icmp' 'interface' 'ip'"
                "'ipv6' 'ldap' 'license' 'logging' 'mac-"
                "address' 'mac-address-table'"
                "'management-access' 'monitor-interface'"
                "'mtu' 'multicast-routing' 'nac-policy'"
                "'name' 'names' 'nat' 'nat-control' 'no'"
                "'ntp' 'object' 'object-group' 'pager'"
                "'passwd' 'password-policy' 'policy-map'"
                "'privilege' 'prompt' 'resource' 'route'"
                "'router' 'same-security-traffic'"
                "'service' 'service-module' 'service-"
                "policy' 'sla' 'smtp-server' 'snmp-map'"
                "'snmp-server' 'ssh' 'ssl' 'static'"
                "'sysopt' 'tcp-map' 'telnet' 'terminal'"
                "'tftp-server' 'threat-detection' 'time-"
                "range' 'timeout' 'tls-proxy' 'tunnel-"
                "group' 'tunnel-group-map' 'url-cache'"
                "'user-identity' 'username' 'virtual'"
                "'vpdn' 'vpn' 'vpn-group-policy' 'webvpn'"
                "'xlate' 'xlate-bypass' <access_group>"
                '<access_list> <access_list_remark>'
                '<domain_name> <ignored> <object_group>'
                '<time_range> <unmatched> <version> ASA'
                'Version [^\\n]* FWSM Version [^\\n]*'
            )
        ...
        with self._choice():
            ...
            with self._option():
                ...
                with self._optional():
                    self._WS_()
                self._NL_()
                self._define(['standby', 'type', 'value'], [])
            with self._option():
                with self._group():
                    self._token('ipv6')
                    self._WS_()
    ...
    def _name_(self):
        ...
        with self._optional():
            ...
            self._define(['description'], [])
        self._NL_()
        self._define(['address', 'description', 'hostname'], [])
        ...
        with self._choice():
            with self._option():
                ...
                with self._optional():
                    ...
                    self._acl_options_()
                    self.name_last_node('options')
                    self._define(['options'], [])
                with self._optional():
                    self._WS_()
                self._NL_()
                self._define(['dst', 'extended', 'icmp', 'id', 'mode', 'options', 'protocol', 'src'], [])
            with self._option():
                self._token('access-list')
                self._WS_()
                self._acl_id_()
                ...
                with self._optional():
                    self._WS_()
                self._NL_()
                self._define(['dst', 'extended', 'id', 'mode', 'options', 'protocol', 'src'], [])
            self._error(
                'expecting one of: '
                "'access-list'"
            )
    ...
    def _access_list_rule_standard_(self):
        ...
        with self._optional():
            self._WS_()
        self._NL_()
        self._define(['extended', 'id', 'mode', 'src'], [])

    @tatsumasu()
    def _access_list_rule_webtype_(self):
        with self._choice():
            with self._option():
                self._token('access-list')
                self._WS_()
                ...
                with self._optional():
                    self._WS_()
                self._NL_()
                self._define(['extended', 'id', 'mode', 'options', 'protocol', 'url'], [])
            with self._option():
                self._token('access-list')
                self._WS_()
                self._acl_id_()
                ...
                with self._optional():
                    self._WS_()
                self._NL_()
                self._define(['dst', 'extended', 'id', 'mode', 'options', 'protocol'], [])
            self._error(
                'expecting one of: '
                "'access-list'"
            )
    ...
    @tatsumasu()
    def _access_list_rule_ethertype_(self):
        self._token('access-list')
        self._WS_()
        self._acl_id_()
        self.name_last_node('id')
    ...
    def _access_list_(self):
        with self._choice():
            with self._option():
                ...
                self._define(['dst', 'extended', 'icmp', 'id', 'mode', 'options', 'protocol', 'remark', 'src'], [])
            with self._option():
                with self._choice():
                    with self._option():
                        self._token('access-list')
            ...
            with self._option():
                ...
                self._NL_()
                self._define(['extended', 'id', 'mode', 'src'], [])
                self._define(['extended', 'id', 'mode', 'remark', 'src'], [])
            with self._option():
                self._token('access-list')
                self._WS_()
                self._acl_id_()
            ...
            with self._option():
                ...
                self._define(['dst', 'extended', 'id', 'mode', 'options', 'protocol', 'remark', 'url'], [])
            with self._option():
                with self._choice():
                    with self._option():
                        self._token('access-list')
            ...
            with self._option():
                ...
                self._NL_()
                self._define(['extended', 'id'], [])
                self._define(['extended', 'id', 'remark'], [])
            with self._option():
                self._token('access-list')
                self._WS_()
                self._acl_id_()
        ...
        with self._choice():
            with self._option():
                ...
                with self._optional():
                    self._WS_()
                self._NL_()
                self._define(['direction', 'iface', 'name', 'type'], [])
            with self._option():
                self._token('access-group')
                self._WS_()
                self._obj_name_()
        ...
        with self._choice():
            with self._option():
                ...
                with self._optional():
                    self._WS_()
                self._NL_()
                self._define(['dst', 'iface', 'options', 'pos', 'service', 'src', 'type'], [])
            with self._option():
                self._constant('None')
                self.name_last_node('type')
                self._token('nat')
    ...
    @tatsumasu()
    def _nat_mapped_src_dynamic_node_(self):
        with self._choice():
            with self._option():
                self._constant('object')
                self.name_last_node('type')
                ...
                with self._optional():
                    ...
                    self._nat_mapped_fallback_()
                self.name_last_node('fallback')
                self._define(['fallback', 'name', 'type'], [])
            with self._option():
                self._constant('group')
                self.name_last_node('type')
                self._acl_object_group_network_()
            ...
            self._error(
                ...
                '<acl_object_group_network_id>'
                '<acl_object_network>'
                '<acl_object_network_id> <pat_pool>'
            )

    @tatsumasu()
    def _nat_real_node_(self):
        with self._choice():
            with self._option():
                with self._if():
                    with self._group():
    ...
    def _nat_options_(self):
        with self._choice():
            ...
            self._error(
                'expecting one of: '
                "'description' 'dns' 'inactive' 'net-to-"
                "net' 'no-proxy-arp' 'route-lookup'"
                "'unidirectional'"
            )
    ...
    def _ignored_(self):
        with self._choice():
            ...
            self._error(
                'expecting one of: '
                "'!' ':' 'Cryptochecksum:' 'aaa' 'aaa-"
                "server' 'app-agent' 'arp' 'arp-"
                "inspection' 'asdm' 'auth-prompt' 'boot'"
                "'call-home' 'class-map' 'clock'"
                "'command-alias' 'console' 'crypto'"
                "'dhcpd' 'dhcprelay' 'dns' 'dns-guard'"
                "'dynamic-access-policy-record' 'enable'"
                "'eou' 'established' 'failover'"
                "'firewall' 'flow-export' 'fragment'"
                "'ftp' 'global' 'group-policy' 'hpm'"
                "'http' 'icmp' 'ip' 'ipv6' 'ldap'"
                "'license' 'logging' 'mac-address' 'mac-"
                "address-table' 'management-access'"
                "'monitor-interface' 'mtu' 'multicast-"
                "routing' 'nac-policy' 'names' 'nat-"
                "control' 'no' 'ntp' 'pager' 'passwd'"
                "'password-policy' 'policy-map'"
                "'privilege' 'prompt' 'resource' 'route'"
                "'router' 'same-security-traffic'"
                "'service' 'service-module' 'service-"
                "policy' 'sla' 'smtp-server' 'snmp-map'"
                "'snmp-server' 'ssh' 'ssl' 'static'"
                "'sysopt' 'tcp-map' 'telnet' 'terminal'"
                "'tftp-server' 'threat-detection'"
                "'timeout' 'tls-proxy' 'tunnel-group'"
                "'tunnel-group-map' 'url-cache' 'user-"
                "identity' 'username' 'virtual' 'vpdn'"
                "'vpn' 'vpn-group-policy' 'webvpn'"
                "'xlate' 'xlate-bypass'"
            )
