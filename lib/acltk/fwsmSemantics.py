...
class fwsmSemantics(aclSemantics, _fwsmSemantics):
    ...
    def access_list(self, ast):
        ...
        assert ast.extended is not None and ast.extended in frozenset(
            ["extended", "standard", "webtype", "ethertype"]
        ), f"unexpected value {ast.type}"
    ...
    def nat(self, ast):
        ...
        r = NATObject(
            NATReal(real["iface"], real["src"], real["dst"], real.get("service", None)),
            NATMapped(mapped["iface"], mapped["src"], mapped["dst"], mapped.get("service", None)),
            description,
            options,
        )
        return r
